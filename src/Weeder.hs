...
-- algebraic-graphs
import Algebra.Graph (Graph, edge, empty, overlay, overlays, star, stars, vertex)
...
import GHC.Iface.Ext.Types (
  ...
  ContextInfo (ClassTyDecl, Decl, EvidenceVarBind, PatternBind, RecField, TyDecl, Use, ValBind),
  DeclType (ClassDec, ConDec, DataDec, FamDec, SynDec),
  EvVarSource (EvInstBind, cls),
  HieAST (Node, nodeChildren, nodeSpan, sourcedNodeInfo),
  HieASTs (HieASTs),
  HieArgs (HieArgs),
  HieFile (HieFile, hie_asts, hie_hs_file, hie_module, hie_types),
  HieType (HAppTy, HCastTy, HCoercionTy, HForAllTy, HFunTy, HLitTy, HQualTy, HTyConApp, HTyVarTy),
  HieTypeFix (Roll),
  IdentifierDetails (IdentifierDetails, identInfo, identType),
  NodeAnnotation (NodeAnnotation, nodeAnnotType),
  NodeInfo (nodeAnnotations, nodeIdentifiers),
...
findNodeTypes t n@Node{nodeChildren, sourcedNodeInfo} =
  if any (any ((t ==) . unpackFS . nodeAnnotType) . nodeAnnotations) (getSourcedNodeInfo sourcedNodeInfo)
    then pure n
    else foldMap (findNodeTypes t) nodeChildren
...
findEvInstBinds n =
  (\(d, ids, ast) -> (d, getClassNames ids, ids, ast))
    <$> findIdentifiers'
      ( not
          . Set.null
          . getEvVarSources
  ...
  where
    ...
    getEvVarSources =
      foldMap (maybe mempty Set.singleton)
        . Set.map \case
  ...
  foldMap
    ( \case
        ...
        (Right name, ids@IdentifierDetails{identInfo}) ->
          if f identInfo
            then (,ids,n) <$> foldMap pure (nameToDeclaration name)
            else mempty
    )
...
requestEvidence n d = do
  ...
  where
    ...
    evidenceUseTree Node{sourcedNodeInfo, nodeChildren} =
      Tree.Node
